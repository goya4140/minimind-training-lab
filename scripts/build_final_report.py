...
def main() -> None:
    ...
    lines = [
        ...
        f"| Video QIVD 留出集 | loss {fmt(video_eval['test_loss'])}; exact {fmt(qivd_generation['normalized_exact_match'])}; contains reference {fmt(qivd_generation['contains_reference'])}; token F1 {fmt(qivd_generation['token_f1'])}; 倒序 F1 {fmt(qivd_generation['reversed_frame_token_f1'])}; 回答变化率 {fmt(qivd_generation['completion_change_rate_on_reversal'])}; 平均生成 {qivd_generation['mean_generation_seconds']:.2f}s/样例 |",
