...
def validation_loss(model, loader, device: torch.device) -> float:
    ...
    for input_ids, labels in loader:
        loss = model(input_ids.to(device), labels.to(device))["loss"]
        if torch.isfinite(loss):
            losses.append(loss.item())
    model.train()
    if not losses:
        raise RuntimeError("validation set produced no finite assistant-token losses")
...
def main() -> None:
    ...
    if not initial_checkpoint.exists():
        raise FileNotFoundError(f"pretraining checkpoint is required: {initial_checkpoint}")
    ...
    for step in range(start_step + 1, total_steps + 1):
        ...
        loss = model(input_ids.to(device), labels.to(device))["loss"] / accumulation
