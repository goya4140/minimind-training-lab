...
由于 SigLIP2 始终冻结，正式训练先把每个视频的 `[8, 64, 768]` patch features 缓存为本地 FP16
memory map。两个训练阶段读取相同特征，不重复进行约五轮视觉编码。缓存只改变计算成本，不参与评估、
不上传 GitHub；最终 Video-QA 评估仍从原始 MP4 解码。
